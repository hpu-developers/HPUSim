"""
  This module implements load_tile unit for HPU
  It is supposed to fetch data from L2 + DRAM and
  writes it to scratchpad
"""
...
class LoadTile(FunctionalUnit):
  ...
  def run_new_task_if_given(self, task):
    ...
    if((task is not None) and (task['accelerator'] == self.name)):
      ...
      if(self.tfu_or_cpu == 'cpu'):
        ## debug
        print("Performing load_tile operation in memory unit!")
        self.task_counter = 0
  ...
  def run_cycle(self, task, ack, abort, AccStatus, cache_controller_resp):
    ...
    if(task[1]):
      ...
      self.request_end = 0
      task[0]['src0_end'] = task[0]['src0'] + task[0]['src1']
      ...
      if((cycles_left == 0) and (self.request_end == 0)):
        ...
        if(self.task['src0'] + 4 < self.task['src0_end']):
          print("Condition check: ", self.task['src0'] + 4, " < ",self.task['src0_end'] )
          self.task['src0'] += 4
...
if __name__ == "__main__":
  ...
  #### CREATE A CPU LOAD TILE TASK ####
  # load_tile 10, 8, R2
  task1 = { 'accelerator' : 'load_tile',
            'src0'        : 10,
            'src1'        : 8,
            'dst'         : 'R2',
            'control'     : 0,
  ...
  # load_tile 100, 16, R3
  task2 = { 'accelerator' : 'load_tile',
             'src0'        : 100,
             'src1'        : 16,
             'dst'         : 'R3',
             'control'     : 0,
             'task_id'     : 5,
             'data'        : {'in': [100, False], 'out': [3]}
           }

  #### CREATE LOAD_TILE UNIT AND MEMORY SUBSYSTEM (L2 + DRAM) ####
  lw = LoadTile('load_tile', 36)
  ...
  #### RUN CYCLE 2 ####
  print("\n\n======== CYCLE 2 ========\n\n")
  req = lw.run_cycle((task1, False), CDB, (None, False), status, cycles_resp_channel[0])
  print("Returned request -- ", req)
  # memory subsystem
  # connect load unit to read_channel0 of the controller
  # NOTE: cycles_resp_channel goes into load unit in the next cycle
  ...
  print("\n\n======== CYCLE 3 ========\n\n")
  req = lw.run_cycle((task2, False), CDB, (None, False),status, cycles_resp_channel[0])
  print("Returned request -- ", req)
  # memory subsystem
  # connect load unit to read_channel0 of the controller
  # NOTE: cycles_resp_channel goes into load unit in the next cycle
  ...
  #### RUN CYCLE 102 ####
  print("\n\n======== CYCLE 102 ========\n\n")
  CDB = ('load_tile', 36, 4, 12, None, True)
  req = lw.run_cycle((None, False), CDB, (None, False), status, cycles_resp_channel[0])
  print("Returned request -- ", req)
  # memory subsystem
  # connect load unit to read_channel0 of the controller
  # NOTE: cycles_resp_channel goes into load unit in the next cycle
