"""
  This module implements a generic load unit for HPU instructions.
"""
...
class Load(FunctionalUnit):
  """
  class for simulating a load unit for HPU instructions.
  """
  ...
  def run_new_task_if_given(self, task):
    """
    This initiates execution of new load in the functional unit and runs a cycle
  ...
  def run_cycle(self, task, ack, abort, AccStatus, cache_controller_resp):
    ...
    if(task[1]):
      ...
      print("request_end: ", self.request_end == 0, ", busy: ", self.busy, ", waiting_for_cycles: ", self.waiting_for_cycles)
...
if __name__ == "__main__":
  ...
  #### CREATE A CPU LOAD TASK ####
  # lw 10, xx, R2
  task1 = { 'accelerator' : 'lw',
            'src0'        : 10,
            'src1'        : 0,
            'dst'         : 'R2',
            'control'     : 0,
  ...
  #### CREATE LOAD UNIT AND MEMORY SUBSYSTEM (L1 + L2 + DRAM) ####
  lw = Load('lw', 32)
