...
class CPUMemoryController:
  ...
  def __init__(self, shmem_controller):
    """
    Initialize controller with an instance of CPUMemory

    Args:
      shmem_controller: SharedMemoryController instance to connect L1 with requisite L2+DRAM subsystem

    Returns:
      None
    """
    # configuration of L1 cache is pre-defined in CPUMemory constructor
    # configuration of L2 cache and DRAM are pre-defined in SharedMemory constructor
    self.bank = CPUMemory(shmem_controller.bank)
  ...
  def run_cycle(self, read_channel0, write_channel0, read_channel1, write_channel1):
    ...
    if(self.channel['write_channel0'] != 0):
      ...
      if(not self.write_queue_0.empty()):
        ...
        # debug
        #print("self.channel[write_channel0] when doing a new store0: ", self.channel['write_channel0'])
...
if __name__ == "__main__":
  ...
  for i in range(0,100):
    ...
    read_resp_channel0, read_resp_channel1, cycles_resp_channel = cpumem_controller.run_cycle(read_channel0, write_channel0, read_channel1, write_channel1)
    print("CYCLE ", cpumem_controller.total_cycles)
    print("READ RESP CHANNEL0 (ID, DATA, VALID): ", read_resp_channel0)
    print("READ RESP CHANNEL1 (ID, DATA, VALID): ", read_resp_channel1)
    print("CYCLES RESP CHANNEL (ID, CYCLES, VALID) X 4: ", cycles_resp_channel)
    print("\n-----------------------------\n\n")
